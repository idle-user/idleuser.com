...
<main role="main" class="container">
    ...
    <div class="my-3 p-3 bg-white rounded shadow-sm">
        <h6 class="border-bottom border-gray pb-2 mb-0">Matches Recently Updated<small class="float-right">limit
                5</small></h6>
        ...
        <?php } else {
            foreach ($recent_match_updates as $rm) {
                ...
                <div class="media text-muted pt-3">
                    <p class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
                        <a class="d-block float-right"
                           href="matches-editor?search=<?php echo $rm['id'] ?>"><?php echo $rm['info_last_updated'] ?></a>
                        <strong class="d-block text-gray-dark"><?php echo $rm['event'] . ' - ' . $rm['title'] . ' - ' . $rm['match_type'] ?></strong>
                        <?php echo $rm['contestants'] ?><small
                                class="float-right"><?php echo $rm['info_last_updated_by_username'] ?></small>
                    </p>
                </div>
                <?php
            }
    ...
    <div class="my-3 p-3 bg-white rounded shadow-sm">
        <h6 class="border-bottom border-gray pb-2 mb-0">Create-a-Poll Recently Created<small class="float-right">limit
                5</small></h6>
        ...
        <?php } else {
            foreach ($recent_polls as $rp) {
                ...
                <div class="media text-muted pt-3">
                    <p class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
                        <a class="d-block float-right"
                           href="/projects/create-a-poll/vote?id=<?php echo $rp['id'] ?>"><?php echo $rp['created_dt'] ?></a>
                        <strong class="d-block text-gray-dark"><?php echo $rp['content'] ?></strong>
                        <?php echo $rp['ending_in'] < 0 ? 'expired' : '' ?><small
                                class="float-right"><?php echo $rp['username'] ?></small>
                    </p>
                </div>
                <?php
            }
            ...
            foreach (array_slice($recent_users, 0, 5) as $ru) {
                ...
                <div class="media text-muted pt-3">
                    <p class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
                        ...
                        <small><?php echo $ru['discord_id'] ? '+Discord' : ' ';
                            echo $ru['chatango_id'] ? '+Chatango' : ' ' ?></small>
                    </p>
                </div>
                <?php
            }
