...
<main role="main" class="container">
    ...
    <div class="my-3 p-3 bg-white rounded shadow-sm">
        <h6 class="border-bottom border-gray pb-2 mb-0">Command
            Editor<?php if ($command_info) echo "<small class='float-right'>Last Updated: $command_info[last_updated]</small>"; ?></h6>
        <div class="media text-muted pt-3">
            ...
            <?php if ($command_info) { ?>
                ...
                <form class="container" method="post">
                    <input type="text" class="form-control d-none" name="command_id"
                           value="<?php echo $command_info['id'] ?>">
                    <input type="text" class="form-control d-none" name="last_updated"
                           value="<?php echo $command_info['last_updated'] ?>">
                    <div class="form-row">
                        <div class="col-md-4 mb-3">
                            <label for="inputCommand">Command</label>
                            <input type="text" class="form-control my-1 mr-sm-2" id="inputCommand" name="command"
                                   placeholder="!command" value="<?php echo $command_info['command'] ?>" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="col-md-12 mb-3">
                            <label for="inputResponse">Response</label>
                            <textarea rows="5" class="form-control my-1 mr-sm-2" id="inputResponse" name="response"
                                      placeholder="Response"><?php echo $command_info['response'] ?></textarea>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="col-md-12 mb-3">
                            <label for="inputDescription">Description</label>
                            <textarea rows="3" class="form-control my-1 mr-sm-2" id="inputDescription"
                                      name="description"
                                      placeholder="Description"><?php echo $command_info['description'] ?></textarea>
                        </div>
                    </div>
                    <button class="btn btn-primary float-right" type="submit">Update Command</button>
                    <a type="button" class="btn btn-primary btn-sm mt-3 float-left" href="discordbot-commands">Create
                        New</a>
                </form>
                ...
                <form class="container" method="post">
                    ...
                    <div class="form-row">
                        <div class="col-md-4 mb-3">
                            <label for="inputCommand">Command</label>
                            <input type="text" class="form-control my-1 mr-sm-2" id="inputCommand" name="command"
                                   placeholder="!command" pattern="^!\S+$" title="Must start with '!' and no spaces."
                                   required>
                        </div>
                    </div>
                    <div class="form-row">
