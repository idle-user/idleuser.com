...
<head>
    ...
    <?php
    $topic_tokens = array_filter(explode(' ', $poll['content']), function ($v) {
        return strlen($v) > 3;
    });
...
<main role="main">
    ...
    <section class="jumbotron text-center bg-light">
        ...
        <div class="pb-4 small container col-md-4">
            ...
            <div class="input-group shadow input-group-sm">
                <input class="form-control form-control-sm" id="share_poll"
                       value="https://<?php echo getenv('DOMAIN') . $_SERVER['REQUEST_URI']; ?>" type="text">
                <button class="btn btn-sm btn-secondary" data-clipboard-target="#share_poll" title="Copy URL"><i
        ...
        <div class="container-fluid col-md-7">
            ...
            <p class="text-muted small"><?php if ($poll['user_id']) {
                    echo "Poll created by $poll[username]";
                } ?></p>
            <div class="card-body">
                ...
                <form class="container pb-5" method="post">
                        ...
                        foreach ($poll['items'] as $item) {
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input"
                                       type="<?php echo $poll['allow_multi'] ? 'checkbox' : 'radio' ?>"
                                       id="<?php echo $item['id'] ?>" name="item[]"
                                       value="<?php echo $item['id'] ?>" <?php echo $voted_on ? 'checked' : ''; ?> <?php echo $already_voted ? 'disabled' : ''; ?>>
                                <label class="form-check-label <?php echo $voted_on ? 'font-weight-bold' : '' ?>"
                                       for="<?php echo $item['id'] ?>"><?php echo $item['content'] ?></label>
                            </div>
                    ...
                    <?php if (!$_SESSION['loggedin'] && !$already_voted) { ?>
                        <div class="form-group row">
                            <div class="col-sm-12">
                                <div class="g-recaptcha" style="text-align: center; display: inline-block;"
                    ...
                    <button class="btn btn-primary" type="submit" <?php echo $already_voted ? 'disabled' : '' ?>
                            id="recaptchaSubmitBtn"><?php echo $already_voted ? 'Already Voted' : 'Submit Vote' ?></button>
                    <div class="pt-2">
                        <small class="text-muted">
                            <text name="countdown" value="<?php echo $poll['ending_in'] ?>"></text>
                        </small>
                    </div>
                </form>
            <?php } ?>
                        ...
                        <div class="col-md-4">
                            <div class="card mb-4 shadow-sm">
                                <div class="card-header text-center m-2"
                                     style="height:225px;background-color: <?php echo random_hex_color(0x7f7f7f) ?>">
                                    <h4 class="text-light"><?php echo $poll['content'] ?></h4>
                                </div>
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="btn-group">
                                            <a href="vote?id=<?php echo $poll['id'] ?>" type="button"
                                               class="btn btn-sm btn-outline-secondary">Vote</a>
                                        </div>
                                        <small class="text-muted">
                                            <text name="countdown" value="<?php echo $poll['ending_in'] ?>"></text>
                                        </small>
    ...
    <section class="jumbotron text-center bg-light">
        <div class="container">
            <h1>Create a Poll</h1>
            <p class="lead text-muted">Pineapple on pizza? Peanut butter on burgers?<br/>Whatever it may be, quickly
                create and share a poll with others!</p>
            <p>
