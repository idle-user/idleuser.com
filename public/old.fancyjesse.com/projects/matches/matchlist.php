...
?>
			<tr>
				...
				?></td>
				<td><?php
					echo $match['title_id'] ? '<b>'.$_SESSION['data']['title'][$match['title_id']]['name'].'</b><br/><br/>':'';
					echo $match['match_type_id'] ? $_SESSION['data']['match_type'][$match['match_type_id']]['name']:'N/A';
...
?>
	...
	<?php	
		if ($current_page != 1) {
			...
			echo "<a href='{$_SERVER['PHP_SELF']}?{$query_result}' class='m-2 btn' >Previous Page</a>";
		}
		...
		if ($current_page != $pages) {
			...
			$query['page'] = $nextpage;
			$query_result = http_build_query($query);
			echo "<a href='{$_SERVER['PHP_SELF']}?{$query_result}' class='m-2 btn '>Next Page</a>";
		}
	?>
	</div>
</div>
